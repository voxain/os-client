...
meowos.ErrorHandler = function(location = "SYSTEM", error, reportToDevs = false) {
  ...
  document.body.innerHTML =
    ...
    + "<div id='errorDiv'>"
      ...
      + "<p><span id='stop'>STOP Code: </span><span>" + errorData.split("\n")[0] + "</span><br><br>In order to continue MeowOS, you will need to reload MeowOS. Any progress on or within programs may have been lost.</p>"
...
meowos.log = function(type, location, message) { // This is crappy code
  var loggingcolors = meowos.system.loggingcolors, typecolor = loggingcolors.FgWhite;

  if (type == "X") typecolor = loggingcolors.FgRed; // X = Error
  else if (type == "i") typecolor = loggingcolors.FgBlue; // i = Information
  else if (type == "!") typecolor = loggingcolors.FgYellow; // ! = Warning
...
programsInstalled.forEach((p) => {
  ...
  $.ajax({
    ...
    error: function (request, status, error) {
      ...
      meowos.ErrorHandler("PROGRAM LOADER", `Failed to load '${p}' due to: ${message}`, true);
    }
  });
});
